...
interface GoogleWorkspaceFolderStepProps {
  account: Account;
  onBack?: () => void;
  onComplete?: (account: Account) => void;
}

export function GoogleWorkspaceFolderStep({ account, onBack, onComplete }: GoogleWorkspaceFolderStepProps) {
  ...
  const fetchFolders = async () => {
    setRefreshing(true);
    try {
      const folderListResult = await window.accountsAPI.listMailboxFolders(account.config);
      const folderList = (folderListResult.folders || []) as { name: string; id: string }[];
      setFolders(folderList);

      // Auto-select Spam or Junk label if available
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl">Select Spam Label</CardTitle>
          <CardDescription>
            Choose which label to move detected spam emails to
          </CardDescription>
        </CardHeader>
          ...
          {loading ? (
            ...
            <>
              <div className="space-y-2">
                <Label htmlFor="spamFolder">Spam/Junk Label</Label>
