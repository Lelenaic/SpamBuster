...
function SettingsContent() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      if (typeof window !== "undefined" && window.aiAPI && window.generalAPI) {
        const [ai, general, mailAccounts] = await Promise.all([
          window.aiAPI.getAll(),
          window.generalAPI.getAll(),
          window.accountsAPI.getAll(),
        ])

        setAiSource(ai.aiSource)
        setOllamaBaseUrl(ai.ollamaBaseUrl)
        setOpenRouterApiKey(ai.openRouterApiKey)
        setSelectedModel(ai.selectedModel)
        setSelectedEmbedModel(ai.selectedEmbedModel)
        setEnableVectorDB(ai.enableVectorDb)
        setCustomizeSpamGuidelines(ai.customizeSpamGuidelines)
        setCustomSpamGuidelines(ai.customSpamGuidelines)
        setTemperature(ai.temperature)
        setTopP(ai.topP)

        setAiSensitivity(general.aiSensitivity)
        setEmailAgeDays(general.emailAgeDays)
        setSimplifyEmailContent(general.simplifyEmailContent)
        setSimplifyEmailContentMode(general.simplifyEmailContentMode)
        setEnableCron(general.enableCron)
        setCronExpression(general.cronExpression)
        setSchedulerMode(general.schedulerMode)
        setSchedulerSimpleValue(general.schedulerSimpleValue)
        setSchedulerSimpleUnit(general.schedulerSimpleUnit)
  ...
  const handleFetchModifyFolders = async () => {
    ...
    try {
      ...
      if (result.success) {
        setModifyFolders((result.folders || []) as { name: string; path: string }[]);
      } else {
