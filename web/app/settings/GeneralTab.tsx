...
export default function GeneralTab({
  ...
  // Skip the first run so we never write on initial mount / first render
  const cronEffectHasRunRef = useRef(false)

  // Effect to update cron when simple schedule values change
  useEffect(() => {
    if (!cronEffectHasRunRef.current) {
      cronEffectHasRunRef.current = true
      return
    }
    const updateCronFromSimple = async () => {
      if (schedulerMode === 'simple' && enableCron && typeof window !== 'undefined' && window.generalAPI) {
        ...
        if (lastCronExprRef.current !== cronExpr) {
          ...
          // Keep local input in sync (do NOT call setCronExpression, which would
          // re-trigger the parent change handler and issue a second PUT)
          setCronInputValue(cronExpr)
        }
      }
    }
  ...
  const handleCronExpressionChange = async (value: string) => {
    ...
    if (typeof window !== "undefined" && window.generalAPI) {
      ...
      if (!validation.valid) {
        const errorMessage = validation.error ? (typeof validation.error === 'string' ? validation.error : (validation.error as Error).message) : "Invalid cron expression"
  ...
  const handleClearChecksums = async () => {
    try {
      ...
      // Clear processed checksums from the local processed-cache
