...
export default function NextAnalysisTimer({ isProcessing }: NextAnalysisTimerProps) {
  ...
  const configRef = useRef<SchedulerConfig | null>(null)
  const nextRunTimeRef = useRef<Date | null>(null)
  const animationRef = useRef<number | null>(null)
  const isProcessingRef = useRef<boolean>(false)
  const initialLoadDone = useRef<boolean>(false)

  // Keep a ref of config so animation/callbacks can read the latest value
  // without depending on `config` (which would recreate the callbacks and
  // retrigger the loadConfig effect in an infinite loop).
  useEffect(() => {
  ...
  const startAnimation = useCallback(() => {
    ...
    const updateDisplay = () => {
      ...
      if (!isProcessingRef.current && nextRunTimeRef.current && configRef.current) {
        ...
        if (nextRunTimeRef.current.getTime() <= Date.now()) {
          const nextRun = calculateNextRun(configRef.current)
  ...
  useEffect(() => {
    const loadConfig = async () => {
      if (typeof window !== 'undefined' && window.generalAPI) {
        const general = await window.generalAPI.getAll([
          'enableCron',
          'schedulerMode',
        ...
        const newConfig = {
          enableCron: general.enableCron,
          schedulerMode: general.schedulerMode,
          schedulerSimpleValue: general.schedulerSimpleValue,
          schedulerSimpleUnit: general.schedulerSimpleUnit,
          cronExpression: general.cronExpression,
        }
        setConfig(newConfig)
        
        // Calculate next run time immediately
        if (newConfig.enableCron) {
    ...
    // Guard the initial load against React Strict Mode's double-invocation in
    // dev. The scheduler-settings-changed listener below still reloads on demand.
