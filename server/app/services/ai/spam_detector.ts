...
import type { EmailData } from '#services/mail/mail_types'
import type { SimilarEmail } from '#services/vector/vector_types'
import type { VectorService } from '#services/vector/vector_types'
 ...
 */
export function decodeBase64IfNeeded(content: string): string {
...
Do not include any other text or formatting.`
  ...
  async analyzeEmail(email: EmailData, rules: Rule[] = []): Promise<SpamAnalysisResult> {
    const simplifyEmailContent = this.config.simplifyEmailContent
    const simplifyEmailContentMode = this.config.simplifyEmailContentMode
    const customizeSpamGuidelines = this.config.customizeSpamGuidelines
    const customSpamGuidelines = this.config.customSpamGuidelines

    // Get similar emails for context (if VectorDB is enabled)
    let similarEmails: SimilarEmail[] = []
    if (this.config.enableVectorDB && this.config.vectorService) {
      try {
        const queryText = `${email.subject} ${email.body || ''}`.substring(0, 1000) // Limit query length
        similarEmails = await this.config.vectorService.findSimilarEmails(queryText, 3) // Get top 3 similar emails
      } catch (error) {
    ...
    const selectedModel = this.config.selectedModel
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const aiResponse = await this.aiService.sendMessage(userPrompt, selectedModel, temperature, topP, systemPrompt)
